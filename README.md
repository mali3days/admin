# Development Plan:
1. Generate uniq ID 
2. Convert uniq ID into QR code 
3. Create Socket Connection (with uniq ID) with Socket Server for Video page 
4. Parse QR with phone camera and redireact to Site Page where uniq ID is a queryParameter
5. Establish new socket connection with SOcket Server from this page with uniq ID 
6. Make iutput element and receive text (html) from the Video page via Socket Server
7. Socket server should broadcast text (html) to all subscribers (exclude Video page, from the one Socket Server only receive the data)
8. TBD
